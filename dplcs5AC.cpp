...
//bool trace[10005][10005]; 
//Chia chieu dau cho 64. 
bitset <10005> trace[10005];
...
int main(){
    ...
    for (int i = 1; i<=s.size(); i++){
        for (int j = 1; j<=t.size(); j++){
            ...
            if (s[i-1] == t[j-1]) dp[1][j] = dp[0][j-1] + 1;
                ...
                if (dp[0][j] > dp[1][j-1]) {
                    ...
                    dp[1][j] = dp[0][j];
                    trace[i][j] = 0; //Rẽ nhánh i
                }

                else {
                    
                    dp[1][j] = dp[1][j-1];
                    trace[i][j] = 1; //Rẽ nhánh j 
                }
        ...
        for (int j = 1; j<=t.size(); j++) dp[0][j] = dp[1][j];    
    }
    ...
    //Lưu ý: Nhớ trừ 1 tại index do lúc nãy đã dịch xâu lên
        ...
        else if (trace[i][j] == 0){ //Kiem tra nhanh i
